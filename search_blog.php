...
<body>
    ...
    <section class="">
        ...
        <div class="blog-container">
            <div class="blogs">
                ...
                echo "<h1 class='search_topic'>Search Topic: $search</h1>";
                $offset = ($page_number - 1) * $limit;
                $select_blogs = $conn->prepare("SELECT * FROM `blog` WHERE title LIKE '%{$search}%' OR advisor_name LIKE '%{$search}%' OR category LIKE '%{$search}%' LIMIT {$offset}, {$limit}");
                ...
                <?php
                    ...
                    echo '<p class="empty">no blog found!</p>';
                }
                ?>
            </div>
    ...
    <?php
    $select_blogs = $conn->prepare("SELECT * FROM `blog` WHERE title LIKE '%{$search}%' OR advisor_name LIKE '%{$search}%' OR category LIKE '%{$search}%'");
    $select_blogs->execute();
    ...
    if ($page_number > 1) {
        echo '<li><a href="search_blog.php?search_box='.$search.'&page=' . ($page_number - 1) . '"><i class="fa-solid fa-chevron-left"></i></a></li>';
    }
    ...
    for ($i = 1; $i <= $total_page; $i++) {
        ...
        echo '<li class=' . $active . '><a href="search_blog.php?search_box='.$search.'&page=' . $i . '">' . $i . '</a></li>';
    }

    if ($total_page > $page_number) {
        echo '<li><a href="search_blog.php?search_box='.$search.'&page=' . ($page_number + 1) . '"><i class="fa-solid fa-chevron-right"></i></a></li>';
    }
